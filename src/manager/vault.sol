 ...
 */
contract Vault is IVault, Initializable, UUPSUpgradeable, IERC1155Receiver {
    // constants
        ...
        returns (bytes32)
    {
        ...
        // Prepare condition in conditional tokens contract
        // Use vault as oracle so that reportPayouts (called from vault) can find the condition
        // The conditionId must match: getConditionId(vault, questionId, outcomeCount)
        conditionalTokens.prepareCondition(address(this), questionId, outcomeCount);

        // Generate condition ID - must match what conditional tokens generates
        // Conditional tokens uses: keccak256(abi.encodePacked(oracle, questionId, outcomeCount))
        // We use vault (address(this)) as oracle so reportPayouts can find it
        bytes32 conditionId = CTHelpersV2.getConditionId(address(this), questionId, outcomeCount);

        questionConditionId[questionId] = conditionId;
        questionOutcomeCount[questionId] = outcomeCount;
        questionOracle[questionId] = address(this); // Store vault as oracle
        questionGame[questionId] = msg.sender; // Store game contract for reference
        ...
        onlyAuthorized(GAME_CONTRACT_ROLE)
    {
        ...
        bytes32 conditionId = questionConditionId[questionId];
        require(conditionId != bytes32(0), "Condition not found");

        uint256 outcomeCount = questionOutcomeCount[questionId];
    ...
    // ) external onlyAuthorized(GAME_CONTRACT_ROLE) {
    //     require(address(conditionalTokens) != address(0), "Conditional tokens not set");
    //     require(!isWithdrawn[questionId], AlreadyWithdrawn(questionId));

    //     bytes32 conditionId = questionConditionId[questionId];
    //     if (conditionId == bytes32(0)) revert ConditionNotPrepared(questionId);

    //     // Use the stored conditionId (already includes game address)
    ...
    //     require(positionBalance > 0, PositionNotFound());
     ...
     */
    function _authorizeUpgrade(address newImplementation) internal override onlyAuthorized(DEFAULT_ADMIN_ROLE) {}

...
get-market network="sepolia":
  ...
  infisical run --path="/sub0contract" -- forge script script/getMarket.s.sol:GetMarket -vvvv \
    --rpc-url {{if network == "sepolia" { rpc_sepolia } else { "http://localhost:8545" } }}

create-market network="sepolia":
  @echo "Creating market on {{network}}..."
  infisical run --path="/sub0contract" -- forge script script/createMarket.s.sol:CreateMarket -vvvv \
    --rpc-url {{if network == "sepolia" { rpc_sepolia } else { "http://localhost:8545" } }} \
    --broadcast
...
allowlist-reporter network="sepolia":
  ...
  infisical run --path="/sub0contract" -- forge script script/allowListReporter.s.sol:AllowListReporter -vvvv \
    --rpc-url {{if network == "sepolia" { rpc_sepolia } else { "http://localhost:8545" } }} \
    --broadcast
...
set-cre-forwarder-config network="sepolia":
  ...
  infisical run --path="/sub0contract" -- forge script script/setCreForwarderConfig.s.sol:SetCreForwarderConfig -vvvv \
    --rpc-url {{if network == "sepolia" { rpc_sepolia } else { "http://localhost:8545" } }} \
    --broadcast
